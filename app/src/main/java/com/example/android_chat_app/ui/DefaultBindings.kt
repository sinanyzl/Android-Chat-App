...
fun bindBlurImageWithPicasso(imageView: ImageView, url: String?){
    if (!url.isNullOrBlank()){
        Picasso.get().load(url).error(R.drawable.ic_baseline_error_24).transform(BlurTransformation(imageView.context, 15, 1)).into(imageView)
    }
}
...
fun TextView.bindEpochTimeMsToDateWithDaysAgo(epochTimeMs: Long){
    ...
    this.text = when{
        ...
        numOfDays > 1.toLong() -> "$numOfDays days ago"
            ...
            val pat = SimpleDateFormat().toLocalizedPattern().replace("\\W?[YyMd]+\\W?".toRegex(), " ")
...
@BindingAdapter("bind_diasble_item_animator")
